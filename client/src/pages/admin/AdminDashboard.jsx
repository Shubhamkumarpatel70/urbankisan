      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-5">
          ...
          {loading ? (
            ...
            <p className="text-brown/50 text-sm py-8 text-center">
              No orders yet
            </p>
          ) : (
              ...
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b border-wheat text-left">
                    <th className="pb-3 pl-5 text-xs font-medium text-brown/50 uppercase">
                      Order ID
                    </th>
                    <th className="pb-3 text-xs font-medium text-brown/50 uppercase">
                      Customer
                    </th>
                    <th className="pb-3 text-xs font-medium text-brown/50 uppercase">
                      Total
                    </th>
                    <th className="pb-3 pr-5 text-xs font-medium text-brown/50 uppercase">
                      Status
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {recentOrders.map((order) => (
                    <tr
                      key={order._id}
                      className="border-b border-wheat/50 last:border-0"
                    >
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          <h3 className="font-display text-lg font-semibold text-brown mb-5">
            Quick Actions
          </h3>
          <div className="space-y-3">
            <Link
            ...
            <Link
              to="/dashboard/site-images"
              className="flex items-center gap-3 p-3 rounded-xl bg-gold/10 hover:bg-gold/20 transition-colors"
            >
              <div className="w-10 h-10 bg-gold/20 rounded-lg flex items-center justify-center">
                <FiImage className="text-gold" size={18} />
              </div>
              <div>
                <p className="text-sm font-medium text-brown">Site Images</p>
                <p className="text-xs text-brown/50">Home Banner & Our Story</p>
              </div>
            </Link>
            <Link
              to="/dashboard/orders"
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-brown/50">
                  {stats.totalCustomers} registered
                </p>
              </div>
            </Link>
          </div>
